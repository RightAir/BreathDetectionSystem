...
# SAME AS V3 BUT WITH ADDED DECISION ALGORITHM TO SEND
    ...
    for i in range(0, len(Class)):
        ...
        if i == 0:
            ...
            if Class[i] != Class[i - 1]:
                ...
                if len(log) > 1 and Class[log[-1]] != Class[log[-2]]\
                and Class[i] != 1:
                    
                    decision.append(Class[i])
                    
                elif len(log) > 1 and Class[log[-1]] == Class[log[-2]]\
                and Class[i] != 1:
